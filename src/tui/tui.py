...
class TorrentTUI:
    ...
    def update(
        ...
        try:
            ...
            if curses.has_colors():
                progress_attr = curses.color_pair(ColorPairs.PROGRESS) | curses.A_BOLD
            else:
                progress_attr = curses.A_BOLD
            ...
            stats_line3 = f" 📥 Downloaded: {format_size(downloaded_bytes)} / {format_size(self.total_bytes)}"
    ...
    def _handle_input(self) -> None:
        ...
        try:
            ...
            if key == curses.KEY_UP or key == ord("k"):
                self.log_scroll_offset = min(self.log_scroll_offset + 1, max(0, len(self.log_buffer) - 1))
            elif key == curses.KEY_DOWN or key == ord("j"):
                self.log_scroll_offset = max(0, self.log_scroll_offset - 1)
            elif key == curses.KEY_PPAGE:  # Page Up
                self.log_scroll_offset = min(self.log_scroll_offset + 10, max(0, len(self.log_buffer) - 1))
