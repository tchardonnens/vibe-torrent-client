...
from peer import MessageType, Peer
from piece_manager import Block, PieceManager, PieceStatus
from torrent_parser import Torrent, TorrentParser
...
class TorrentClient:
    ...
    def _decrement_peer_availability(self, peer: Peer) -> None:
        ...
        for piece_index in peer.counted_pieces:
            if 0 <= piece_index < len(self.piece_availability):
                self.piece_availability[piece_index] = max(0, self.piece_availability[piece_index] - 1)
            ...
            try:
                ...
                for peer_info in peers:
                    ...
                    if peer_key not in self.peers:
                        peer = Peer(
                            ip=peer_info["ip"], port=peer_info["port"], info_hash=self.info_hash, peer_id=self.peer_id
                        )
    ...
    async def _handle_peer(self, peer_key: str, peer: Peer) -> None:
        ...
        try:
            ...
            try:
                msg_type, payload = await asyncio.wait_for(peer.receive_message(), timeout=10.0)
            ...
            receiver_task = asyncio.create_task(self._message_receiver(peer, message_queue, piece_waiters))
    ...
    async def _message_receiver(
        self, peer: Peer, message_queue: asyncio.Queue, piece_waiters: dict[tuple[int, int], asyncio.Future]
    ) -> None:
        """Background task to receive and route messages from peer."""
        while self.downloading and peer.connected:
            try:
                msg_type, payload = await asyncio.wait_for(peer.receive_message(), timeout=1.0)
    ...
    async def _download_from_peer(
        ...
        while self.downloading and peer.connected:
            ...
            while len(download_tasks) < self.max_concurrent_pieces_per_peer:
                piece = self.piece_manager.get_next_piece_to_download(peer_pieces, availability=self.piece_availability)
                if not piece:
                    break
                ...
                task = asyncio.create_task(self._download_piece(peer, piece, message_queue, piece_waiters))
            ...
            done, pending = await asyncio.wait(download_tasks, return_when=asyncio.FIRST_COMPLETED)
        ...
        if download_tasks:
            ...
            for task, result in zip(download_tasks, results, strict=False):
    ...
    async def _download_piece(
        self, peer: Peer, piece, message_queue: asyncio.Queue, piece_waiters: dict[tuple[int, int], asyncio.Future]
    ) -> tuple[bool, int]:
        """
        ...
        async def drain_message_queue_for_piece() -> None:
            ...
            while True:
                ...
                await self.piece_manager.add_block_data(queued_piece_index, block_offset, block_data)
        ...
        try:
            ...
            while True:
                ...
                if not wait_futures:
                    ...
                    continue
                done, pending = await asyncio.wait(wait_futures, return_when=asyncio.FIRST_COMPLETED, timeout=30.0)
                ...
                for future in done:
                    try:
                        ...
                        if key and key in pending_blocks:
                            ...
                            # Add block data
                            await self.piece_manager.add_block_data(piece_index, block_offset, block_data)
