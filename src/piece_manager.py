...
class PieceManager:
    ...
    def get_next_piece_to_download(
        self,
        peer_has_pieces: Set[int],
        availability: Optional[List[int]] = None
    ) -> Optional[Piece]:
        """
        ...
        if availability:
            ordered = sorted(
                available_pieces,
                key=lambda idx: (availability[idx] if idx < len(availability) else 0, idx)
            )
        else:
    ...
    async def assemble_piece(self, piece_index: int) -> Optional[bytes]:
        """
    ...
    async def get_completed_bytes(self) -> int:
        ...
        async with self.piece_lock:
            return sum(
                self.pieces[index].length
                for index in self.completed_pieces
                if index in self.pieces
            )
    
    async def get_next_block_to_request(self, piece_index: int) -> Optional[Block]:
        """
